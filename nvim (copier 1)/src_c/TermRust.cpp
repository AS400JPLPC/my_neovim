...
/// if WORKPGM  = "Pcurs" this lib --> ENV = export PATH=$PATH:$HOME/????
...
bool ctrlPgm(std::string v_TEXT)
{
	...
	std::filesystem::path p(v_TEXT.c_str());
			switch(strswitch(p.stem().c_str()))
				{
					case  strswitch("Ptest")		: b_pgm =true;		break;  // Test module Term
//                    case  strswitch("Pforms")		: b_pgm =true;		break;  // test new module Forms
				}
	return b_pgm;
}
